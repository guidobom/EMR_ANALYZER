...
_TEXTUAL_VALUE = (
    ...
    r"DEBOLE|NEGATIVITA|POSITIVITA)"
)
...
# Fallback pattern for textual results like "NEGATIVO", "POSITIVO"
_TEXTUAL_PAYLOAD_RE = re.compile(
    rf"^\s*(?P<value_text>{_TEXTUAL_VALUE})\s*"
    rf"(?P<explicit_flag>\*{{1,3}}|!|H|L|↑|↓)?\s*"
    rf"(?:(?P<unit>{_RESULT_UNIT})\s*)?"
    rf"(?:\(?\s*(?P<ref_range>{_REFERENCE})\s*\)?)?"
    rf"(?:\s+.*)?$",
...
_NO_COLON_RESULT_RE = re.compile(
    ...
    rf"(?P<payload>(?:[<>]=?\s*)?(?:{_NUMBER}|{_TEXTUAL_VALUE}).*)$",
    re.IGNORECASE,
)
...
class LabParser:
    ...
    def _parse_result_line(
        ...
        # Try numeric payload first, then textual
        payload_match = _PAYLOAD_RE.match(payload)
        textual_match = None if payload_match else _TEXTUAL_PAYLOAD_RE.match(payload)
        if not payload_match and not textual_match:
            return None
        ...
        if payload_match:
            ...
            numeric_str = payload_match.group("value")
            try:
                value = self.normalizer.normalize_value(numeric_str, unit_raw)
            except (TypeError, ValueError):
                return None

        elif textual_match:
            unit_raw = (textual_match.group("unit") or "").strip()
            reference_text = (
                textual_match.group("ref_range") or ""
            ).strip()
            explicit_flag = (
                textual_match.group("explicit_flag") or ""
            ).strip()
            value_text_result = self.normalizer.normalize_textual_value(
                textual_match.group("value_text") or ""
            )
    ...
    def _deduplicate(self, lab_values: list[LabValue]) -> list[LabValue]:
        ...
        for lv in lab_values:
            # Use value_text as part of the key for textual results
            key = (lv.normalized_name, lv.value, lv.value_text, lv.unit)
