...
class PatientRepository:
    ...
    def insert(self, patient: Patient) -> None:
        self.db.execute(
            """INSERT INTO patients (id, pseudonym, sex, birth_year,
               main_pathology, center, notes, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (patient.id, patient.pseudonym, patient.sex, patient.birth_year,
    ...
    def update(self, patient: Patient) -> None:
        self.db.execute(
            """UPDATE patients SET pseudonym=?, sex=?, birth_year=?,
               main_pathology=?, center=?, notes=?, updated_at=?
               WHERE id=?""",
            (patient.pseudonym, patient.sex, patient.birth_year,
             patient.main_pathology, patient.center, patient.notes,
    ...
    def list_all(self) -> list[Patient]:
        ...
        return [
            Patient(
                id=r["id"], pseudonym=r["pseudonym"], sex=r["sex"],
                birth_year=r["birth_year"], main_pathology=r["main_pathology"],
