...
@dataclass
class ClinicalStateDelta:
    """Proposed changes to a Clinical State after new document processing."""
    ...
    confirm: list[dict] = field(default_factory=list)
    conflict: list[dict] = field(default_factory=list)
    ignore_as_duplicate: list[dict] = field(default_factory=list)

    @property
    def low_risk_changes(self) -> list[dict]:
        """Changes that can be applied automatically (confirm, ignore)."""
        return self.confirm + self.ignore_as_duplicate

    @property
    def needs_review(self) -> list[dict]:
        """Changes requiring human validation."""
        return self.add + self.update + self.close + self.conflict

    @property
    def has_changes(self) -> bool:
        return any([
            self.add, self.update, self.close, self.confirm,
            self.conflict, self.ignore_as_duplicate,
        ])
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "ignore_as_duplicate": self.ignore_as_duplicate,
        }

    @classmethod
    def from_dict(cls, data: dict) -> "ClinicalStateDelta":
        return cls(
            add=data.get("add", []),
            ...
            ignore_as_duplicate=data.get("ignore_as_duplicate", []),
        )
