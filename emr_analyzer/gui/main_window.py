...
from ..config import APP_NAME, APP_VERSION, WORKSPACES_DIR
...
class MainWindow(QMainWindow):
    ...
    def _on_open_workspace(self):
        directory = QFileDialog.getExistingDirectory(
            self, "Apri Workspace Paziente", str(WORKSPACES_DIR)
        )
        if directory:
    ...
    def _open_llm_config(self) -> None:
        ...
        try:
            available_models = QwenClient.list_available_models()
    ...
    def _apply_llm_configs(self, configs) -> None:
        ...
        for role in ("document", "clinical_state"):
            config = configs[role]
            client = QwenClient(config=config) if config.model else None
    ...
    def _propagate_state_llm(self, client):
        """Update only services that operate on the longitudinal state."""
        cs_manager = self._services.get("cs_manager")
        if cs_manager is not None:
            cs_manager._qwen_client = client
