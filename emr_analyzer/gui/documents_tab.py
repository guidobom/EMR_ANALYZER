...
class DocumentsTab(QWidget):
    ...
    def _refresh_table(self):
        ...
        self._drop_zone.setVisible(len(docs) == 0)
    ...
    def _process_documents(self, doc_ids: list[str],
        ...
        if llm_only:
            # LLM-only: reuse persisted parsing output.
    ...
    def _process_next_document(self, doc_ids: list[str], index: int,
        ...
        if llm_only:
            ...
            source_candidates.append(
                WORKSPACES_DIR / self._current_patient_id / "docling" /
                f"{doc_id}.md"
            )
            source_path = next(
    ...
    def _run_extraction(self, doc, text: str, parsing_result,
        ...
        events = []
        if skip_llm:
            progress.add_log(f"  ⏭️  LLM saltato (parsing only)")
        else:
    ...
    def _get_extraction_dir(self) -> 'Path':
        ...
        from ..config import WORKSPACES_DIR
        return WORKSPACES_DIR / self._current_patient_id / "extraction"
